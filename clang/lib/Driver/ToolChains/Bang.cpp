...
BangInstallationDetector::BangInstallationDetector(
  ...
  for (const auto &Candidate : Candidates) {
    ...
    char* strCmd = "cnmon | grep "MLU[0-9][0-9][0-9]" -o";
    FILE* fp = popen(strCmd, "r"); // 执行命令并打开命令输出管道
    if (fp == NULL) {
        llvm::out<<"Failed to run bang arch command\n";
        exit(1);
    }
    
    while (fgets(buffer, sizeof(buffer), fp)) {
        llvm::out<<"Notice : bang arch is "<<buffer<<"\n"; // 输出命令执行结果
    }
    ...
    std::string GpuArchName(CudaArchToVirtualArchString(GpuArch));

    llvm::out<<"MLU GpuArchName is "<<GpuArchName<<"\n";

    if (GpuArchName >= "mlu200")
      // __BANG_ARCH__ >= 200
...
void BangInstallationDetector::print(raw_ostream &OS) const {
  auto it = LibDeviceMap.begin();
  auto arch_num = it->first;
  if (isValid())
    OS << "Found BANG installation: " << InstallPath << ", libdevice arch : "
       << arch_num << "\n";
}
...
void MLISA::Assembler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // Pass -v to cnas if it was passed to the driver.
...
static bool shouldIncludeMLISA(const ArgList &Args, const char *gpu_arch) {
  ...
  for (Arg *A : Args) {
    ...
    if (ArchStr == "all" || ArchStr == gpu_arch) {
      includePTX = A->getOption().matches(options::OPT_bang_include_mlisa_EQ);
      continue;
    }
  }
  return includeMLISA;
