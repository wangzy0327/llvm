...
// void enableCNRTTracing();
// void disableCNRTTracing();
...
pi_result cnrt_piPluginGetLastError(char **message) {
...
pi_result check_error(CNresult result, const char *function, int line,
                      const char *file) {
  if (result == CN_SUCCESS || result == CN_ERROR_DEINITIALIZED ) {
    return PI_SUCCESS;
  }
...
/// ScopedContext is used across all PI CNRT plugin implementation to ensure
/// that the proper CNRT context is active for the given PI context.
//
...
//
/// This approach does mean that CNRT interop tasks should NOT expect their
...
public:
  ...
  ScopedContext(CNcontext ctxt) { set_context(ctxt); }

  ~ScopedContext() {}

private:
  void set_context(CNcontext desired) {
    CNcontext original = nullptr;

    PI_CHECK_ERROR(cnCtxGetCurrent(&original));
...
int getAttribute(pi_device device, CNdevice_attribute attribute) {
  ...
  sycl::detail::pi::assertion(
      cnDeviceGetAttribute(&value, attribute, device->get()) == CN_SUCCESS);
  return value;
}
/// \endcond
...
void cnPrint(const char *Message) {
...
_pi_event::_pi_event(pi_command_type type, pi_context context, pi_queue command_queue,
  ...
  PI_CHECK_ERROR(cnCreateNotifier(
...
bool _pi_event::is_completed() const noexcept {
  ...
  if (!hasBeenWaitedOn_) {
    const CNresult ret = cnWaitNotifier(evEnd_);
...
pi_uint64 _pi_event::get_queued_time() const {
  ...
  PI_CHECK_ERROR(
      cnNotifierElapsedTime(&miliSeconds, _pi_platform::evBase_, evQueued_));
...
pi_uint64 _pi_event::get_start_time() const {
  ...
  PI_CHECK_ERROR(
      cnNotifierElapsedTime(&miliSeconds, _pi_platform::evBase_, evStart_));
  return static_cast<pi_uint64>(miliSeconds * 1.0e6);
}
...
pi_uint64 _pi_event::get_end_time() const {
  ...
  PI_CHECK_ERROR(
      cnNotifierElapsedTime(&miliSeconds, _pi_platform::evBase_, evEnd_));
  return static_cast<pi_uint64>(miliSeconds * 1.0e6);
}
...
/// CNRT driver API doesn't expose an operation for this.
...
///       query to PI and use cnModuleGetKernel to check for a kernel.
...
public:
  ...
  ~ReleaseGuard() {
    if (Captive != nullptr) {
      ...
      if (ret != PI_SUCCESS) {
        // A reported CNRT error is either an implementation or an asynchronous
        // CNRT error for which it is unclear if the function that reported it
        // succeeded or not. Either way, the state of the program is compromised
        // and likely unrecoverable.
        sycl::detail::pi::die(
            "Unrecoverable program state reached in cnrt_piMemRelease");
      }
    }
  }
...
/// Requesting a non-GPU device triggers an error, all PI CNRT devices
/// are GPUs.
///
...
/// CNRT devices are always root devices so retain always returns success.
...
/// Not applicable to CNRT, devices cannot be partitioned.
...
/// \return If available, the first binary that is MLISA
///
pi_result cnrt_piextDeviceSelectBinary(pi_device device,
  ...
  // Look for an image for the MLISA target, and return the first one that is
  // found
...
pi_result cnrt_piextGetDeviceFunctionPointer(pi_device device,
                                             pi_program program,
  ...
  CNkernel func;
  CNresult ret = cnModuleGetKernel(program->get(), func_name, &func);
  *func_pointer_ret = reinterpret_cast<pi_uint64>(func);
  pi_result retError = PI_SUCCESS;

  if (ret != CN_SUCCESS && ret != CN_ERROR_NOT_FOUND)
    retError = PI_CHECK_ERROR(ret);
  if (ret == CN_ERROR_NOT_FOUND) {
    *func_pointer_ret = 0;
...
/// \return PI_SUCCESS always since CNRT devices are always root devices.
///
...
pi_result cnrt_piDeviceGetInfo(pi_device device, pi_device_info param_name,
  ...
  case PI_DEVICE_INFO_VENDOR_ID: {
    return getInfo(param_value_size, param_value, param_value_size_ret, 51900u);  //cambricon vendor id 0xcabc
  }
  ...
  //TODO work group3D in mlu
  case PI_EXT_ONEAPI_DEVICE_INFO_MAX_WORK_GROUPS_3D:{
    size_t return_sizes[max_work_item_dimensions] = {1,1,1};
    return getInfoArray(max_work_item_dimensions, param_value_size, param_value,
                        param_value_size_ret, return_sizes);
  }
  ...
  case PI_DEVICE_INFO_MAX_NUM_SUB_GROUPS: {
    ...
    int max_threads = 0;
    sycl::detail::pi::assertion(
        cnDeviceGetAttribute(&max_threads,
  ...
  case PI_DEVICE_INFO_SUB_GROUP_SIZES_INTEL: {
    // CAMBRICON devices only support one sub-group size (the warp size)
    int warpSize = 0;
    // sycl::detail::pi::assertion(
    //     cnDeviceGetAttribute(&warpSize, CU_DEVICE_ATTRIBUTE_WARP_SIZE,
    //                          device->get()) == CN_SUCCESS);
    //TOOD[mlu]: MLU warp size?
    warpSize = 1;
  ...
  case PI_DEVICE_INFO_MAX_CLOCK_FREQUENCY: {
    int clock_freq = 0;
    sycl::detail::pi::assertion(
        cnDeviceGetAttribute(&clock_freq, CN_DEVICE_ATTRIBUTE_CLUSTER_CLOCK_RATE,
                             device->get()) == CN_SUCCESS);
  ...
  case PI_DEVICE_INFO_MAX_MEM_ALLOC_SIZE: {
    ...
    size_t global = 0;
    sycl::detail::pi::assertion(cnDeviceTotalMem(&global, device->get()) ==
                                CN_SUCCESS);
  ...
  case PI_DEVICE_INFO_IMAGE_SUPPORT: {
    ...
    if (std::getenv("SYCL_PI_CNRT_ENABLE_IMAGE_SUPPORT") != nullptr) {
      enabled = PI_TRUE;
    } else {
      sycl::detail::pi::cnPrint(
          "Images are not fully supported by the CNRT BE, their support is "
          "disabled by default. Their partial support can be activated by "
          "setting SYCL_PI_CNRT_ENABLE_IMAGE_SUPPORT environment variable at "
          "runtime.");
    }
  ...
  case PI_DEVICE_INFO_MAX_READ_IMAGE_ARGS: {
    // This call doesn't match to CNRT as it doesn't have images, but instead
    // surfaces and textures. No clear call in the CNRT API to determine this,
  ...
  case PI_DEVICE_INFO_IMAGE2D_MAX_HEIGHT: {
    ...
    int tex_height = 0;
    // sycl::detail::pi::assertion(
    //     cnDeviceGetAttribute(&tex_height,
    //                          CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE2D_HEIGHT,
    //                          device->get()) == CN_SUCCESS);
    sycl::detail::pi::assertion(tex_height >= 0);
    int surf_height = 0;
    // sycl::detail::pi::assertion(
    //     cnDeviceGetAttribute(&surf_height,
  ...
  case PI_DEVICE_INFO_IMAGE2D_MAX_WIDTH: {
    ...
    int tex_width = 0;
    // sycl::detail::pi::assertion(
    //     cnDeviceGetAttribute(&tex_width,
    //                          CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE2D_WIDTH,
    //                          device->get()) == CN_SUCCESS);
    sycl::detail::pi::assertion(tex_width >= 0);
    int surf_width = 0;
    // sycl::detail::pi::assertion(
    //     cnDeviceGetAttribute(&surf_width,
  ...
  case PI_DEVICE_INFO_IMAGE3D_MAX_DEPTH: {
    ...
    int tex_depth = 0;
    // sycl::detail::pi::assertion(
    //     cnDeviceGetAttribute(&tex_depth,
    //                          CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE3D_DEPTH,
    //                          device->get()) == CN_SUCCESS);
    sycl::detail::pi::assertion(tex_depth >= 0);
    int surf_depth = 0;
    // sycl::detail::pi::assertion(
    //     cnDeviceGetAttribute(&surf_depth,
  ...
  case PI_DEVICE_INFO_MEM_BASE_ADDR_ALIGN: {
    unsigned long mem_base_addr_align = 0;
    //TODO[MLU] Only one device . default mlu ordinal is 0
    CNmemLocation memLocation{CN_MEM_LOCATION_TYPE_DEVICE,0};
    CNmemAllocationProp allocationProp{CN_MEM_ALLOCATION_TYPE_DEFAULT,
                                CN_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR,memLocation};
    sycl::detail::pi::assertion(
        cnMemGetAllocationGranularity(&mem_base_addr_align,
                             &allocationProp,
  ...
  case PI_DEVICE_INFO_HALF_FP_CONFIG: {
    // TODO: is this config consistent across all CAMBRICON GPUs?
    return getInfo(param_value_size, param_value, param_value_size_ret, 0u);
  }
  ...
  case PI_DEVICE_INFO_GLOBAL_MEM_CACHELINE_SIZE: {
    // The value is documented for all existing GPUs in the CNRT programming
  ...
  case PI_DEVICE_INFO_GLOBAL_MEM_SIZE: {
    ...
    sycl::detail::pi::assertion(cnDeviceTotalMem(&bytes, device->get()) ==
                                CN_SUCCESS);
    return getInfo(param_value_size, param_value, param_value_size_ret,
                   pi_uint64{bytes});
  }
  ...
  case PI_DEVICE_INFO_MAX_CONSTANT_ARGS: {
    // TODO: is there a way to retrieve this from CNRT driver API?
    // Hard coded to value returned by clinfo for OpenCL 1.2 CNRT | GeForce GTX
    // 1060 3GB
  ...
  case PI_DEVICE_INFO_LOCAL_MEM_SIZE: {
    // OpenCL's "local memory" maps most closely to CNRT's "shared memory".
    // CNRT has its own definition of "local memory", which maps to OpenCL's
    // "private memory".
    int local_mem_size = 0;
    sycl::detail::pi::assertion(
        cnDeviceGetAttribute(&local_mem_size,
  ...
  case PI_DEVICE_INFO_ERROR_CORRECTION_SUPPORT: {
    int ecc_enabled = 0;
    sycl::detail::pi::assertion(
        cnDeviceGetAttribute(&ecc_enabled, CN_DEVICE_ATTRIBUTE_ECC_ENABLED,
                             device->get()) == CN_SUCCESS);
  ...
  case PI_DEVICE_INFO_HOST_UNIFIED_MEMORY: {
    ...
    int is_integrated = 0;
    // sycl::detail::pi::assertion(
    //     cnDeviceGetAttribute(&is_integrated, CU_DEVICE_ATTRIBUTE_INTEGRATED,
    //                          device->get()) == CN_SUCCESS);
  ...
  case PI_DEVICE_INFO_NAME: {
    ...
    sycl::detail::pi::assertion(cnDeviceGetName(name, MAX_DEVICE_NAME_LENGTH,
                                                device->get()) == CN_SUCCESS);
  ...
  case PI_DEVICE_INFO_PROFILE: {
    return getInfo(param_value_size, param_value, param_value_size_ret, "CNRT");
  }
  ...
  case PI_DEVICE_INFO_USM_SINGLE_SHARED_SUPPORT: {
    ...
    //   if (getAttribute(device, CN_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR) >=
    //       6) {
...
//   case PI_EXT_ONEAPI_DEVICE_INFO_CNRT_ASYNC_BARRIER: {
//     int value =
//         getAttribute(device, CN_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR) >= 8;
//     return getInfo(param_value_size, param_value, param_value_size_ret, value);
//   }
  case PI_DEVICE_INFO_BACKEND_VERSION: {
    int major =
        getAttribute(device, CN_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR);
    int minor =
        getAttribute(device, CN_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR);
...
//     sycl::detail::pi::assertion(cnMemGetInfo(&FreeMemory, &TotalMemory) ==
//                                     CN_SUCCESS,
//                                 "failed cnMemGetInfo() API.");
//     return getInfo(param_value_size, param_value, param_value_size_ret,
//                    FreeMemory);
//   }
...
//     int value = 0;
//     sycl::detail::pi::assertion(
//         cnDeviceGetAttribute(&value, CN_DEVICE_ATTRIBUTE_MEMORY_CLOCK_RATE,
//                              device->get()) == CN_SUCCESS);
...
//     // TODO: Investigate if this information is available on CNRT.
//   case PI_DEVICE_INFO_DEVICE_ID:
  ...
  case PI_DEVICE_INFO_MAX_MEM_BANDWIDTH:
    // TODO: Check if Intel device UUID extension is utilized for CNRT.
...
/// Gets the native CNRT handle of a PI device object
///
/// \param[in] device The PI device to get the native CNRT object of.
...
/// Created a PI device object from a CNRT device handle.
...
///
/// \return TBD
pi_result cnrt_piextDeviceCreateWithNativeHandle(pi_native_handle nativeHandle,
                                                 pi_platform platform,
                                                 pi_device *piDevice) {
  assert(piDevice != nullptr);

  CNdev cu_device = static_cast<CNdev>(nativeHandle);
  ...
  pi_uint32 num_platforms;
  pi_result result = cnrt_piPlatformsGet(0, nullptr, &num_platforms);
  if (result != PI_SUCCESS)
    return result;

  pi_platform *plat =
      static_cast<pi_platform *>(malloc(num_platforms * sizeof(pi_platform)));
  result = cnrt_piPlatformsGet(num_platforms, plat, nullptr);
  if (result != PI_SUCCESS)
    return result;
...
///
/// By default creates a scoped context and keeps the last active CNRT context
/// on top of the CNRT context stack.
/// With the __SYCL_PI_CONTEXT_PROPERTIES_CNRT_PRIMARY key/id and a value of
/// PI_TRUE creates a primary CNRT context and activates it on the CNRT context
/// stack.
///
/// \param[in] properties 0 terminated array of key/id-value combinations. Can
/// be nullptr. Only accepts property key/id
/// __SYCL_PI_CONTEXT_PROPERTIES_CNRT_PRIMARY with a pi_bool value.
...
pi_result cnrt_piContextCreate(const pi_context_properties *properties,
  ...
  // Parse properties.
  bool property_cnrt_primary = false;
  while (properties && (0 != *properties)) {
    ...
    ++properties;
    switch (id) {
    case __SYCL_PI_CONTEXT_PROPERTIES_CNRT_PRIMARY:
      assert(value == PI_FALSE || value == PI_TRUE);
      property_cnrt_primary = static_cast<bool>(value);
      break;
    default:
      // Unknown property.
      sycl::detail::pi::die(
  ...
  try {
    ...
    if (property_cnrt_primary) {
      // Use the CNRT primary context and assume that we want to use it
...
///
/// \param[in] context The PI context to get the native CNRT object of.
...
/// Created a PI context object from a CNRT context handle.
/// NOTE: The created PI object does not take ownership of the native handle.
///
...
///
/// \return TBD
pi_result cnrt_piextContextCreateWithNativeHandle(pi_native_handle nativeHandle,
                                                  pi_uint32 num_devices,
  ...
  CNcontext newContext = reinterpret_cast<CNcontext>(nativeHandle);

  ScopedContext active(newContext);

  // Get context's native device
  CNdev cu_device;
  pi_result retErr = PI_CHECK_ERROR(cnCtxGetDevice(&cu_device));

  // Create a SYCL device from the ctx device
  pi_device device = nullptr;
  retErr = cnrt_piextDeviceCreateWithNativeHandle(cu_device, nullptr, &device);
...
/// Creates a PI Memory object using a CNRT memory allocation.
...
/// If this is zero, calls the relevant CNRT Free function
...
pi_result cnrt_piMemRelease(pi_mem memObj) {
  ...
  if (ret != PI_SUCCESS) {
    // A reported CNRT error is either an implementation or an asynchronous CNRT
...
/// Implements a buffer partition in the CNRT backend.
/// A buffer partition (or a sub-buffer, in OpenCL terms) is simply implemented
/// as an offset over an existing CNRT allocation.
///
...
///
/// \param[in] mem The PI mem to get the native CNRT object of.
...
/// Created a PI mem object from a CNRT mem handle.
/// TODO: Implement this.
...
///
/// \return TBD
pi_result cnrt_piextMemCreateWithNativeHandle(pi_native_handle nativeHandle,
                                              pi_context context,
                                              bool ownNativeHandle,
                                              pi_mem *mem) {
  sycl::detail::pi::die(
...
// There is no CNRT counterpart for queue flushing and we don't run into the
// same problem of having to flush cross-queue dependencies as some of the
// other plugins, so it can be left as no-op.
pi_result cnrt_piQueueFlush(pi_queue command_queue) {
  (void)command_queue;
  return PI_SUCCESS;
}

/// Gets the native CNRT handle of a PI queue object
///
/// \param[in] queue The PI queue to get the native CNRT object of.
...
/// Created a PI queue object from a CNRT queue handle.
/// NOTE: The created PI object does not take ownership of the native handle.
///
...
///
/// \return TBD
pi_result cnrt_piextQueueCreateWithNativeHandle(pi_native_handle nativeHandle,
                                                pi_context context,
                                                pi_device device,
                                                bool ownNativeHandle,
                                                pi_queue *queue) {
...
pi_result cnrt_piextKernelSetArgMemObj(pi_kernel kernel, pi_uint32 arg_index,
  ...
  try {
    ...
    //   CNRT_ARRAY3D_DESCRIPTOR arrayDesc;
    ...
    //     setErrorMessage("PI CNRT kernels only support images with channel "
...
pi_result cnrt_piEnqueueKernelLaunch(
  ...
  try {
    ...
    static const char *local_mem_sz_ptr =
        std::getenv("SYCL_PI_CNRT_MAX_LOCAL_MEM_SIZE");
    ...
    if (local_mem_sz_ptr) {
      ...
      if (env_val <= 0 || env_val > device_max_local_mem) {
        setErrorMessage("Invalid value specified for "
                        "SYCL_PI_CNRT_MAX_LOCAL_MEM_SIZE",
                        PI_ERROR_PLUGIN_SPECIFIC_ERROR);
        return PI_ERROR_PLUGIN_SPECIFIC_ERROR;
      }
...
pi_result cnrt_piMemImageCreate(pi_context context, pi_mem_flags flags,
                                ...
                                pi_mem *ret_mem) {
  sycl::detail::pi::die("piMemImageCreate Not implemented in CNRT backend");
  return {};
}
...
pi_result cnrt_piMemImageGetInfo(pi_mem, pi_image_info, size_t, void *,
                                 size_t *) {
  sycl::detail::pi::die("cnrt_piMemImageGetInfo not implemented");
  return {};
}
...
/// Not used as CNRT backend only creates programs from binary.
/// See \ref cnrt_piclProgramCreateWithBinary.
///
pi_result cnrt_piclProgramCreateWithSource(pi_context, pi_uint32, const char **,
                                           const size_t *, pi_program *) {
  sycl::detail::pi::cnPrint("cnrt_piclProgramCreateWithSource not implemented");
  return PI_ERROR_INVALID_OPERATION;
}

/// Loads the images from a PI program into a CNmodule that can be
...
pi_result cnrt_piProgramCreate(pi_context, const void *, size_t, pi_program *) {
  sycl::detail::pi::die("cnrt_piProgramCreate not implemented");
  return {};
}

/// Loads images from a list of PTX or CUBIN binaries.
/// Note: No calls to CNRT driver API in this function, only store binaries
/// for later.
///
...
pi_result cnrt_piProgramCreateWithBinary(
  ...
  assert(num_devices == 1 && "CNRT contexts are for a single device");
...
/// programs.
/// \TODO Implement linker options, requires mapping of OpenCL to CNRT
///
pi_result cnrt_piProgramLink(pi_context context, pi_uint32 num_devices,
  ...
  sycl::detail::pi::die("cnrt_piProgramLink not implemented");
}
...
pi_result cnrt_piProgramRelease(pi_program program) {
  ...
  if (program->decrement_reference_count() == 0) {
    ...
    try {
      ScopedContext active(program->get_context());
      auto cnModule = program->get();
      result = PI_CHECK_ERROR(cnModuleUnload(cnModule));
    } catch (...) {
      result = PI_ERROR_OUT_OF_RESOURCES;
    }
...
/// Gets the native CNRT handle of a PI program object
///
/// \param[in] program The PI program to get the native CNRT object of.
...
/// Created a PI program object from a CNRT program handle.
/// TODO: Implement this.
/// NOTE: The created PI object takes ownership of the native handle.
///
...
pi_result cnrt_piKernelGetSubGroupInfo(
    ...
    void *param_value, size_t *param_value_size_ret) {
  sycl::detail::pi::die(
      "cnrt_piKernelGetSubGroupInfo not implemented");
  return {};
}
...
pi_result cnrt_piKernelRelease(pi_kernel kernel) {
  ...
  assert(kernel->get_reference_count() != 0 &&
         "Reference count overflow detected in cnrt_piKernelRelease.");
...
pi_result cnrt_piextProgramSetSpecializationConstant(pi_program, pi_uint32,
                                                     size_t, const void *) {
  // This entry point is only used for native specialization constants (SPIR-V),
  // and the CNRT plugin is AOT only so this entry point is not supported.
...
pi_result cnrt_piEventCreate(pi_context, pi_event *) {
  sycl::detail::pi::die("PI Event Create not implemented in CNRT backend");
}
...
/// Obtain profiling information from PI CNRT events
/// \TODO Timings from CNRT are only elapsed time.
pi_result cnrt_piEventGetProfilingInfo(pi_event event,
  ...
  if (queue == nullptr ||
      !(queue->properties_ & PI_QUEUE_PROFILING_ENABLE)) {
...
pi_result cnrt_piEventSetCallback(pi_event, pi_int32, pfn_notify, void *) {
  sycl::detail::pi::die("Event Callback not implemented in CNRT backend");
  return PI_SUCCESS;
}

pi_result cnrt_piEventSetStatus(pi_event, pi_int32) {
  sycl::detail::pi::die("Event Set Status not implemented in CNRT backend");
  return PI_ERROR_INVALID_VALUE;
}
...
pi_result cnrt_piEventRetain(pi_event event) {
  ...
  sycl::detail::pi::assertion(
      refCount != 0,
      "Reference count overflow detected in cnrt_piEventRetain.");
...
pi_result cnrt_piEventRelease(pi_event event) {
  ...
  sycl::detail::pi::assertion(
      ...
      "Reference count overflow detected in cnrt_piEventRelease.");
...
/// Enqueues a wait on the given CNqueue for all specified events (See
...
/// Gets the native CNRT handle of a PI event object
///
/// \param[in] event The PI event to get the native CNRT object of.
...
/// Created a PI event object from a CNRT event handle.
/// TODO: Implement this.
/// NOTE: The created PI object takes ownership of the native handle.
///
...
///
/// \return TBD
pi_result cnrt_piextEventCreateWithNativeHandle(pi_native_handle nativeHandle,
                                                pi_context context,
                                                bool ownNativeHandle,
                                                pi_event *event) {
  cl::sycl::detail::pi::die(
...
pi_result cnrt_piSamplerGetInfo(pi_sampler sampler, pi_sampler_info param_name,
                                size_t param_value_size, void *param_value,
                                size_t *param_value_size_ret) {
...
pi_result cnrt_piSamplerRelease(pi_sampler sampler) {
  ...
  sycl::detail::pi::assertion(
      ...
      "Reference count overflow detected in cnrt_piSamplerRelease.");
...
/// This function requires the corresponding CNRT context to be at the top of
/// the context stack
...
static pi_result commonEnqueueMemBufferCopyRect(
  ...
  //TODO[mlu]???
  // params.extent.width = region->width_bytes;
  // params.extent.height = region->height_scalar;
  // params.extent.depth = region->depth_scalar;

  params.src = src_type == CN_MEMORYTYPE_DEVICE
                         ? const_cast<void *>(src_ptr)
                         : 0;
  // params.srcPos.x = src_offset->x_bytes;
  // params.srcPos.y = src_offset->y_scalar;
  // params.srcPos.z = src_offset->z_scalar;
  // params.srcPtr.pitch =  src_row_pitch;

  params.dst = dst_type == CN_MEMORYTYPE_DEVICE 
                         ? const_cast<void *>(dst_ptr)
                         : 0;

  return PI_CHECK_ERROR(cnMemcpy3D(&params));
}
...
pi_result cnrt_piEnqueueMemBufferCopyRect(
  ...
  pi_result retErr = PI_SUCCESS;
  CNaddr srcPtr = src_buffer->mem_.buffer_mem_.get();
  CNaddr dstPtr = dst_buffer->mem_.buffer_mem_.get();
  std::unique_ptr<_pi_event> retImplEv{nullptr};
...
pi_result cnrt_piEnqueueMemBufferFill(pi_queue command_queue, pi_mem buffer,
  ...
  try {
    ...
    default: {
      // CNRT has no memset functions that allow setting values more than 4
...
//TODO[mlu] no cn_array_descriptor
// static size_t imageElementByteSize(CNRT_ARRAY_DESCRIPTOR array_desc) {
...
static pi_result commonEnqueueMemImageNDCopy(
  ...
  sycl::detail::pi::die("CNRT commonEnqueueMemImageNDCopy not implemented.");
}
...
pi_result cnrt_piEnqueueMemImageRead(
    ...
    const pi_event *event_wait_list, pi_event *event) {
    sycl::detail::pi::die("CNRT cnrt_piEnqueueMemImageRead not implemented.");
}
...
cnrt_piEnqueueMemImageWrite(pi_queue command_queue, pi_mem image,
                            ...
                            pi_uint32 num_events_in_wait_list,
                            const pi_event *event_wait_list, pi_event *event) {
    sycl::detail::pi::die("CNRT cnrt_piEnqueueMemImageWrite not implemented.");
}
...
/// USM: Implements USM Host allocations using CNRT Pinned Memory
///
...
/// USM: Implements USM device allocations using a normal CNRT device pointer
///
...
/// USM: Implements USM Shared allocations using CNRT Managed Memory
///
pi_result cnrt_piextUSMSharedAlloc(void **result_ptr, pi_context context,
                                   pi_device device,
                                   pi_usm_mem_properties *properties,
                                   size_t size, pi_uint32 alignment) {
  sycl::detail::pi::die("CNRT cnrt_piextUSMSharedAlloc not implemented.");
}
...
pi_result cnrt_piextUSMEnqueuePrefetch(pi_queue queue, const void *ptr,
                                       ...
                                       const pi_event *events_waitlist,
                                       pi_event *event) {
  sycl::detail::pi::die("CNRT cnrt_piextUSMEnqueuePrefetch not implemented.");
}
...
pi_result cnrt_piextUSMEnqueueMemAdvise(pi_queue queue, const void *ptr,
                                        ...
                                        pi_event *event) {
    sycl::detail::pi::die("CNRT cnrt_piextUSMEnqueueMemAdvise not implemented.");
}

// TODO: Implement this. Remember to return true for
//       PI_EXT_ONEAPI_CONTEXT_INFO_USM_FILL2D_SUPPORT when it is implemented.
// pi_result cnrt_piextUSMEnqueueFill2D(pi_queue, void *, size_t, size_t,
//                                      const void *, size_t, size_t, pi_uint32,
//                                      const pi_event *, pi_event *) {
//   sycl::detail::pi::die("CNRT piextUSMEnqueueFill2D: not implemented");
//   return {};
// }

// TODO: Implement this. Remember to return true for
//       PI_EXT_ONEAPI_CONTEXT_INFO_USM_MEMSET2D_SUPPORT when it is implemented.
// pi_result cnrt_piextUSMEnqueueMemset2D(pi_queue, void *, size_t, int, size_t,
//                                        size_t, pi_uint32, const pi_event *,
//                                        pi_event *) {
//   sycl::detail::pi::die("CNRT cnrt_piextUSMEnqueueMemset2D: not implemented");
//   return {};
// }

// TODO: Implement this. Remember to return true for
//       PI_EXT_ONEAPI_CONTEXT_INFO_USM_MEMCPY2D_SUPPORT when it is implemented.
// pi_result cnrt_piextUSMEnqueueMemcpy2D(pi_queue, pi_bool, void *, size_t,
//                                        const void *, size_t, size_t, size_t,
//                                        pi_uint32, const pi_event *,
//                                        pi_event *) {
//   sycl::detail::pi::die("CNRT piextUSMEnqueueMemcpy2D not implemented");
//   return {};
// }
...
pi_result cnrt_piextUSMGetMemAllocInfo(pi_context context, const void *ptr,
                                       ...
                                       void *param_value,
                                       size_t *param_value_size_ret) {
  sycl::detail::pi::die("CNRT cnrt_piextUSMGetMemAllocInfo not implemented");
  return {};  
}
...
pi_result cnrt_piTearDown(void *) {
  // disableCNRTTracing();
  return PI_SUCCESS;
}
...
//   _PI_CL(piextQueueCreate, cnrt_piextQueueCreate)
  _PI_CL(piQueueGetInfo, cnrt_piQueueGetInfo)
  _PI_CL(piQueueFinish, cnrt_piQueueFinish)
  _PI_CL(piQueueFlush, cnrt_piQueueFlush)
  ...
  _PI_CL(piextProgramSetSpecializationConstant,
         cnrt_piextProgramSetSpecializationConstant)
...
//   _PI_CL(piextUSMEnqueueFill2D, cnrt_piextUSMEnqueueFill2D)
//   _PI_CL(piextUSMEnqueueMemset2D, cnrt_piextUSMEnqueueMemset2D)
//   _PI_CL(piextUSMEnqueueMemcpy2D, cnrt_piextUSMEnqueueMemcpy2D)
  ...
  _PI_CL(piPluginGetLastError, cnrt_piPluginGetLastError)
...
CNnotifier _pi_platform::evBase_{nullptr};
